...
def test(fusion_model_path=''):
    ...
    class BiAttentionConfig:
        def __init__(self, layer):
            ...
            self.bi_hidden_size = 128
            self.bi_num_attention_heads = 8
            self.v_hidden_size = 512
            if layer == 1:
    ...
    bi_config1 = BiAttentionConfig(1)
    bi_config2 = BiAttentionConfig(2)
    ...
    with torch.no_grad():
        for it, (img_vis, img_ir, vitext, irtext, name) in enumerate(test_bar):
            ...
            _, out= fusionmodel(image_vis_y, img_ir, vitext, irtext)
